...
# === Manajemen Anggota ===
    ...
    if st.session_state.username == "admin":
        ...
        st.header("Daftar Acara & Absensi")
        for i, ac in enumerate(acara):
            ...
            if st.button(f"✏️ Edit {key}"):
                new_judul = st.text_input("Edit Judul", ac["judul"], key=f"ej{i}")
                new_waktu = st.text_input("Edit Waktu", ac["waktu"], key=f"ew{i}")
                new_kode = st.text_input("Edit Kode", ac["kode"], key=f"ek{i}")
                if st.button(f"Simpan {key}"):
