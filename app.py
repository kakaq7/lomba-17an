...
# ====== MANAJEMEN ANGGOTA ======
    ...
    acara = st.text_input("Nama Acara (misal: Rapat 10 Agustus)")
    nama = st.text_input("Nama Anggota")
    if st.button("✅ Absen Hadir"):
        if acara not in absensi_data:
            absensi_data[acara] = []
        if nama and nama not in absensi_data[acara]:
            absensi_data[acara].append(nama)
            save_absensi(absensi_data)
            st.success(f"Anggota '{nama}' tercatat hadir di '{acara}'")
    ...
    for event, hadir in absensi_data.items():
        ...
        for orang in hadir:
            st.write(f"- {orang}")
￼Enter                if st.button("Simpan Juara"):
                    pemenang = []
                    for j in [juara1, juara2, juara3]:
                        if j and j not in pemenang:
                            pemenang.append(j)
